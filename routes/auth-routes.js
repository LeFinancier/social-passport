...
authRoutes.post ('/signup', (req, res, next) => {
  ...
  if (zxcvbn(password).score < 1) {
    res.render('auth/signup', {
      message: `Tu password se adivinó en ${zxcvbn(password).guesses} intentos.`
    })
    return
  }
  ...
  .then(user => {
    if(user !== null) {
      res.render('auth/signup', {
        message: 'El usuario registrado YA existe' 
      })
      return
    }
    ...
    newUser.save((err) => {
      if (err) {
        res.render('auth/signup', {
          message: 'Algo salió mal y no pude guardar tu registro. Inténtalo de nuevo más tarde'
        })
      } else {
        res.redirect('/')
      }
